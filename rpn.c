...
int parenthesisOperation(Stack operands,Stack operators,int index,char* expression) {
	...
	rank>=0 && !rank && infixDigitOperation(i+3,expression,operands) && push(operands,&expression[i+1]) && (i+=4) ||
...
int rankWiseOperation(int rank,Stack operands,Stack operators,int index,char* expression) {
	if(*operators.top)
		precedence(*((char*)(*(operators.top))->data))==precedence(expression[index]) &&
		precedence(expression[index])<2 && push(operands,pop(operators));
